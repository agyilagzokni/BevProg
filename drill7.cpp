...
const string sqrtkey = "sqrt"; //primary
const string powkey = "**"; //primary
	...
	switch (ch)
	{
    	...
    	default: 
    	{
    		if (isalpha(ch))
    		{
    			...
    			else if (s == sqrtkey) return Token{sqrtc};
    			else if (s == powkey) return Token{powc};
...
double calc_sqrt(){
	char ch;
	if (cin.get(ch) && ch != '(') error ("(-t kene irni");
	cin.putback(ch);
	double d = primary();
	if (d < 0) error("nem jo mert negativ");
	return sqrt(d);
}
...
/*double calc_pow(){
	...
	double d = primary();
	t = ts.get();
	if (t.kind != ',') error("hianyzik egy ,");
	t = ts.get();
	double exp = primary();
	t = ts.get();
	if (t.kind != ')') error(")-t kene irni");
	return pow(d, exp);
}*/
